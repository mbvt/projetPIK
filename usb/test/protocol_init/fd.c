...
int main ()
{
  ...
  result = libusb_get_device_list(NULL, &devList);
  if (result >= 0)
  {
    ...
    if (devHandle != NULL)
    {
      ...
      libusb_detach_kernel_driver(devHandle, INTERFACE_NUMBER);
      {
        result = libusb_claim_interface(devHandle, INTERFACE_NUMBER);
        if (result >= 0)
        {
          device_ready = 1;
          struct libusb_control_setup       *Setup;
          struct libusb_transfer            *transfer;                                  
          //transfer = malloc(sizeof (struct libusb_transfer));                         
          transfer = libusb_alloc_transfer(60);                                         
          transfer->buffer = malloc( 128 * sizeof (unsigned
                char));
